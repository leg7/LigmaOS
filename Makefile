...
CFLAGS     = -std=c2x -ffreestanding -nostdlib -I '$(SRC_DIR)/kernel' -I '$(SRC_DIR)/kernel/library' -I '$(SRC_DIR)/libc' -I '$(SRC_DIR)/libc/libk' -Wall -Wextra -Wpedantic -Wvla -Wimplicit-fallthrough -fanalyzer -ggdb
LDFLAGS    = -ffreestanding -nostdlib -lgcc -ggdb
SRC_C      := $(shell find $(SRC_DIR) -type f -name '*.c')
OBJ_C      := $(patsubst $(SRC_DIR)/%.c, $(OBJ_DIR)/%.o, $(SRC_C))

# TODO: Also compile nasm64.asm files if compiliting 64 bit os
